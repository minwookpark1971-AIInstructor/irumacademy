...
-- ⚠ 이 함수는 여기 적혀만 있고 실제 생성은 007 에서 한다.
--   본문이 marketing_consent_log 에 insert 하므로 이 파일 시점에는 만들 수 없다.
--   (007 이 같은 정의를 create or replace 로 다시 선언한다 — 고칠 때 두 곳을 함께 볼 것)
-- 트리거 등록도 마찬가지로 007 끝에서 한다.
